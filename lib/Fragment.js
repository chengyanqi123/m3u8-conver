...
import { findArithmeticProgressionMax, isFunction } from "../utilities.js"
        ...
        for (let i = this.index; i < leng; i++) {
            ...
            if (encryption) {
                await streamPipeline(
                    got.stream(uri, this.options.requestOptions),
                    fs.createWriteStream(encodePath)
                )
            ...
            isFunction(onchange) && onchange(leng, i, fragment)
        }
    }
    merge() {
        return new Promise((resolve, reject) => {
